...
if ($env:OS -ne "Windows_NT") { throw "The Copilot Harness CodeGraph bootstrap currently supports Windows only." }
...
if (-not $SkipInstall -and $null -eq $codegraphCommand) {
    ...
    try {
        ...
        $expectedRepoDeclaration = '$repo = ''colbymchenry/codegraph'''
        if ($installerText -notmatch [regex]::Escape($expectedRepoDeclaration)) {
        ...
        try {
            if (-not [string]::IsNullOrWhiteSpace($Version)) { $env:CODEGRAPH_VERSION = $Version }
            if ($PSCmdlet.ShouldProcess($targetRoot, "Install CodeGraph from $repositoryUrl")) { & $tempInstaller }
        } finally { $env:CODEGRAPH_VERSION = $previousVersion }
    } finally { Remove-Item -LiteralPath $tempInstaller -Force -ErrorAction SilentlyContinue }
    $codegraphCommand = Resolve-CodeGraphCommand
}

if ($null -eq $codegraphCommand) { throw "CodeGraph CLI is not available. Install it from $repositoryUrl and rerun." }
...
if (-not $KeepTelemetry -and $PSCmdlet.ShouldProcess($targetRoot, "Disable CodeGraph telemetry")) {
...
if (-not $SkipInit) {
    ...
    if (Test-Path -LiteralPath $marker) { Write-Host "CodeGraph is already initialized in this repository; leaving the existing index in place." }
